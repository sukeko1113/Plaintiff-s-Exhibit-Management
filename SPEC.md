...
- モデル: `claude-sonnet-4-6`(環境変数 `ANTHROPIC_MODEL` で切替可能)
- API キー: 環境変数 `ANTHROPIC_API_KEY`(`.env` で管理、`.gitignore` 必須)
- システムプロンプトは `app/prompts/extract_metadata.txt` に外出し
  (プロンプト改善時にコード変更不要)
- レスポンスは JSON 形式で返すよう指示し、パース失敗時はリトライしない
  (`AiParseError` として上位層に送出、上位層で 502 へ変換)
- タイムアウト 60 秒、リトライ最大 2 回(指数バックオフ:2秒 → 4秒)。
  リトライ対象は `429`(レート制限)とネットワークエラーのみ。
  認証エラー(401)・タイムアウト・パース失敗はリトライしない。
- 入力トークン上限 30,000(環境変数 `ANTHROPIC_MAX_INPUT_TOKENS` で上書き可)。
  超過時は冒頭 70% / 末尾 30% の比率で切り詰める
  (標目・作成年月日・作成者は冒頭に、署名・日付は末尾に出やすいため)。
  トークン換算は日本語想定の概算式(1 トークン ≒ 2.5 文字)で代用する。
