...
弁護士・パラリーガル等の実務担当者が、ローカル PC 上の訴訟記録フォルダを起点に、甲号証リストの編集・個別マスタの結合・結合済みファイルの分解までをワンストップで行えることを目的としています。
...
- **フォルダ構成の自動セットアップ**
  - ルート直下に「甲号証リスト.docx」「個別マスタ」フォルダ「結合甲号証」フォルダが無ければ自動生成 (`POST /api/setup`)
- **甲号証リストを Word で開く**
  - OS の既定アプリで「甲号証リスト.docx」を起動 (`POST /api/open-list`)
- **結合(マージ)**
  - 「個別マスタ」フォルダ内の各甲号証 `.docx` と「甲号証リスト.docx」を、`docxcompose` を用いて 1 つの Word ファイルに結合 (`POST /api/merge`)
  - リストに無いファイル・マスタに無い項目を警告として通知
- **分解(スプリット)**
...
- **シングルページ UI**
  - `app/static/index.html` から成る React ベースの 1 画面 UI を `GET /` で配信
...
### フロントエンド
- React(`app/static/index.html` にバンドル済みのシングルページ)
...
- **OS**: Windows 10 / 11 を主対象としています
  ...
  - 「甲号証リストを開く」機能は Windows では `os.startfile`、macOS では `open`、Linux では `xdg-open` を内部的に使用します(macOS / Linux でも起動自体は可能)
- **Python**: 3.10 以上を推奨
...
`pytest.ini` の設定に従い、`tests/` 配下のテスト(11 件)が実行されます。
...
│   └── static/index.html    # React ベースのシングルページ UI
├── tests/                   # pytest テスト一式
├── requirements.txt
├── pytest.ini
...
- UI(JSX 構造・className・レイアウト・配色・余白)および既存 API(10 エンドポイント)は維持対象です。変更が必要な場合は `CLAUDE.md` のルールに従い、事前にユーザー承認を得てください。
