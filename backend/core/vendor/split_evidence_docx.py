...
-----
コマンドライン:
    python split_evidence_docx.py 結合ファイル.docx 出力フォルダ/

    # 乙号証用にカスタマイズ:
    python split_evidence_docx.py 結合乙.docx out/ \\
...
埋め込みsectPrの「昇格」処理について
--------------------------------------
WordのOOXML仕様では:
* 文書には1つ以上のセクションが存在する。
* 最後のセクションの設定は <w:body> 直下の <w:sectPr> で記述される。
* それ以外のセクションは、その最後の段落の <w:pPr> に <w:sectPr> を「埋め込む」
  ことで終端を表現する。

分割後の各ファイルは「1つのセクション」のみを含むので、埋め込みsectPrを
body直下に移動して、有効な単一セクション文書とする必要がある。
"""
...
# 【甲第XX号証】 をマッチする正規表現。
# 甲・第・号・証 の各文字間、および XX の前後の空白を許容。
# XX は半角または全角の数字 1 文字以上。第1グループに番号を捕捉する。
DEFAULT_MARKER_PATTERN = r'【\s*甲\s*第\s*([0-9０-９]+)\s*号\s*証\s*】'

# 出力ファイル名のテンプレート。{id} に番号(2桁ゼロ詰め)が入る。
DEFAULT_FILENAME_TEMPLATE = '甲第{id:0>2}号証.docx'

# 万一 sectPr が一切無かった場合のフォールバック(A4縦・1cm余白)。
...
def _normalize_id(id_str: str) -> str:
    """全角数字を半角に変換し、不要なゼロ詰めを除く(後でtemplateで整形)。"""
    table = str.maketrans('０１２３４５６７８９', '0123456789')
    return id_str.translate(table).lstrip('0') or '0'
...
def _find_top_level_spans(body_xml: str) -> list[dict]:
    ...
    XMLコメントやCDATAは document.xml の本文ではほぼ出現しないため考慮しない。
    """
    spans: list[dict] = []
    ...
    while pos < len(body_xml):
        ...
        if is_close:
            # 直近の開始タグと一致する閉じタグなら pop。
            if stack and stack[-1][0] == tag_name:
                _, start = stack.pop()
                if not stack:  # トップレベルに戻った瞬間に1要素確定。
                    spans.append({
                        'tag': tag_name,
                        'start': start,
                        'end': m.end(),
...
def _promote_embedded_sectpr(section_xml: str) -> tuple[str, str | None]:
    ...
    # body直下用にインデントを軽く調整(失敗しても valid な XML にはなる)。
...
def split_docx(
    ...
    marker_pattern : 区切りマーカーの正規表現。第1グループに番号を捕捉すること。
    filename_template : 出力ファイル名のテンプレート。{id} で番号を参照。
    ...
    # 2. <w:body> ... </w:body> を切り出す。
    m = re.search(r'(<w:body\b[^>]*>)(.*)(</w:body>)', doc_xml, re.DOTALL)
    if not m:
        raise ValueError("<w:body> が document.xml に見つかりません。")
    pre_body  = doc_xml[:m.end(1)]                # ヘッダ + <w:body>
    body_inner = m.group(2)                       # 本文
    post_body = doc_xml[m.start(3):]              # </w:body> + 末尾

    # 3. body直下の要素を取得。
    spans = _find_top_level_spans(body_inner)

    # 4. 末尾の body-level sectPr を分離(最後の要素が sectPr ならそれ)。
    ...
    section_anchors: list[tuple[int, str]] = []  # [(span_idx, normalized_id), ...]
    for i, span in enumerate(spans[:last_idx_for_content]):
        ...
        if mm:
            section_anchors.append((i, _normalize_id(mm.group(1))))

    if not section_anchors:
        raise ValueError(
            f"マーカーが見つかりません。pattern={marker_pattern!r}\n"
            f"document.xml の冒頭テキスト: "
            f"{_extract_paragraph_text(spans[0]['xml']) if spans else '(空)'}"
        )

    if verbose:
        print(f"検出: {len(section_anchors)} 件のセクション")
        for _, sid in section_anchors:
            print(f"  - 第{sid}号証")

    # 6. 各セクションごとに新しい docx を生成。
    output_files: list[Path] = []

    for n, (start_idx, sec_id) in enumerate(section_anchors):
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            '例:\n'
            '  python split_evidence_docx.py 結合甲.docx out/\n'
            '  python split_evidence_docx.py 結合乙.docx out/ '
            "--pattern '【\\s*乙\\s*第\\s*([0-9０-９]+)\\s*号\\s*証\\s*】' "
            "--filename '乙第{id:0>2}号証.docx'\n"
        ),
    ...
    p.add_argument('--pattern', default=DEFAULT_MARKER_PATTERN,
                   help='マーカーの正規表現(第1グループに番号)。'
                        f'既定: {DEFAULT_MARKER_PATTERN}')
    p.add_argument('--filename', default=DEFAULT_FILENAME_TEMPLATE,
                   help='出力ファイル名テンプレート。'
                        f'既定: {DEFAULT_FILENAME_TEMPLATE}')
